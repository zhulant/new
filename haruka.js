...
function title() {
    console.log(chalk.bold.green(figlet.textSync('ZhuL Bot', {
        font: 'Standard',
    ...
    console.log(chalk.yellow(`\n${chalk.yellow('Created By ZhuL')}\n`))
}

    const haruka = harukaConnect({
        logger: pino({ level: 'silent' }),
        printQRInTerminal: true,
        browser: ['ZhuL Multi Device','Safari','1.0.0'],
        auth: state
    })
	title() 
    ...
    haruka.ev.on('connection.update', async (update) => {
        ...
        console.log('Connect, Gunakan Dengan Bijak')
    })
